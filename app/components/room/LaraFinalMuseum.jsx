...
function getBrassTexture() {
  ...
  brassTexture = new THREE.CanvasTexture(canvas); brassTexture.colorSpace = THREE.SRGBColorSpace; brassTexture.wrapS = brassTexture.wrapT = THREE.RepeatWrapping; brassTexture.anisotropy = 4;
  return brassTexture;
}
...
function getStoneTexture() {
  ...
  const canvas = document.createElement('canvas'); canvas.width = canvas.height = 256; const ctx = canvas.getContext('2d');
  ctx.fillStyle = '#101214'; ctx.fillRect(0, 0, 256, 256);
  for (let index = 0; index < 72; index += 1) { const x = (index * 61) % 256; const y = (index * 89) % 256; ctx.fillStyle = index % 5 ? 'rgba(245,236,220,.025)' : 'rgba(183,138,88,.16)'; ctx.fillRect(x, y, 1 + (index % 3), 54 + (index % 7) * 10); }
  stoneTexture = new THREE.CanvasTexture(canvas); stoneTexture.colorSpace = THREE.SRGBColorSpace; stoneTexture.wrapS = stoneTexture.wrapT = THREE.RepeatWrapping; stoneTexture.anisotropy = 4;
  return stoneTexture;
}

function getVelvetTexture() {
  if (velvetTexture) return velvetTexture;
  const canvas = document.createElement('canvas'); canvas.width = canvas.height = 256;
  const ctx = canvas.getContext('2d'); ctx.fillStyle = '#342317'; ctx.fillRect(0, 0, 256, 256);
  [[34,35,16],[89,27,11],[151,48,20],[214,32,14],[52,98,14],[122,92,21],[199,106,16],[25,168,19],[92,160,13],[159,172,23],[225,182,16],[53,226,14],[125,221,18],[193,232,12]].forEach(([x,y,r], index) => {
  ...
  velvetTexture = new THREE.CanvasTexture(canvas); velvetTexture.colorSpace = THREE.SRGBColorSpace; velvetTexture.wrapS = velvetTexture.wrapT = THREE.RepeatWrapping; velvetTexture.repeat.set(1.4, 2.1); velvetTexture.anisotropy = 4;
  return velvetTexture;
}

function Brass({ color = BRONZE, roughness = .18 }) {
  const brushed = getBrassTexture();
...
function Velvet() {
  const texture = getVelvetTexture();
...
function MuseumLighting() {
  ...
  return <group name="lara-bright-luxury-museum-lighting">
    ...
    <MuseumSpot position={[0,4.72,-6.85]} target={[0,3.4,ROOM.backWallZ+.34]} intensity={9.5} angle={.48} />
...
function GrandStage() { const z=ROOM.backWallZ+1.22; return <group name="lara-grand-circular-exhibition-pedestal"><mesh position={[0,2.47,ROOM.backWallZ+.16]}><boxGeometry args={[7.9,4.75,.26]} /><Stone color="#171a1d" /></mesh><mesh position={[0,2.48,ROOM.backWallZ+.32]}><boxGeometry args={[5.86,3.58,.08]} /><Velvet /></mesh><mesh position={[0,2.48,ROOM.backWallZ+.39]}><boxGeometry args={[4.95,2.78,.04]} /><Stone color="#121316" /></mesh><ClawAndIvy /><mesh position={[0,.2,z]}><cylinderGeometry args={[2.62,2.82,.34,80]} /><Stone /></mesh><mesh position={[0,.4,z]}><cylinderGeometry args={[2.2,2.44,.09,80]} /><Brass color="#543827" /></mesh><mesh position={[0,.49,z]}><cylinderGeometry args={[1.8,2.04,.14,80]} /><Stone color="#181a1c" /></mesh><mesh position={[0,.58,z]} rotation={[Math.PI/2,0,0]}><torusGeometry args={[1.66,.034,10,80]} /><Brass color={GOLD} /></mesh><mesh position={[0,.65,z]}><cylinderGeometry args={[.31,.37,.1,40]} /><Stone color="#1a1c1f" /></mesh><mesh position={[0,.66,z]}><cylinderGeometry args={[.035,.052,.42,22]} /><Brass /></mesh><mesh position={[0,1.0,z]}><capsuleGeometry args={[.12,.24,8,18]} /><meshPhysicalMaterial color="#ded7ce" metalness={.84} roughness={.23} /></mesh><pointLight position={[0,.82,z+.25]} color="#f5bb71" intensity={3.2} distance={5} decay={1.65} /></group>; }

function MuseumWalls() { return <group name="lara-unique-wall-museum-sections"><MuseumCase side={-1} z={1.25} width={2.1} height={3.95} title="jewelry archive"><JewelCase /></MuseumCase><MuseumCase side={-1} z={-3.05} width={2.55} height={4.18} title="stage wardrobe" velvet><Wardrobe /></MuseumCase><MuseumCase side={-1} z={-7.05} width={1.72} height={3.7} title="fashion sketch"><Boots /></MuseumCase><MuseumCase side={1} z={5.35} width={2.55} height={4.2} title="platinum archive"><MusicArchive /></MuseumCase><MuseumCase side={1} z={-3.35} width={2.3} height={3.95} title="concert archive"><Awards /></MuseumCase><MuseumCase side={1} z={-7.0} width={1.8} height={3.55} title="lyrics archive" velvet><MusicArchive /></MuseumCase></group>; }

export default function LaraFinalMuseum() { return <group name="lara-fifty-million-fashion-museum"><NeroMarquinaFloor /><LaraRunnerCarpet /><HotelCeiling /><MuseumWalls /><GrandStage /><MuseumLighting /><MemberStoryProps member="lara" /></group>; }
